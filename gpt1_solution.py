...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.W_q = nn.Linear(self.total_size, self.total_size, bias=True)
        self.W_k = nn.Linear(self.total_size, self.total_size, bias=True)
        self.W_v = nn.Linear(self.total_size, self.total_size, bias=True)
        self.W_o = nn.Linear(self.total_size, self.total_size, bias=True)
    ...
    def get_attention_weights(self, queries, keys):
        ...
        causal_mask = torch.tril(torch.ones(sequence_length, sequence_length, device=queries.device)).unsqueeze(0).unsqueeze(0)
        attention_scores = attention_scores.masked_fill(causal_mask == 0, float(-1e4))
    ...
    def split_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        batch_size, sequence_length, total_dim = tensor.size()
        dim_per_head = total_dim // self.num_heads
        tensor = tensor.view(batch_size, sequence_length, self.num_heads, dim_per_head)
    ...
    def merge_heads(self, tensor):
        ...
        tensor = tensor.transpose(1, 2)  
        merged_tensor = tensor.contiguous().view(batch_size, sequence_length, num_heads * dim)
    ...
    def forward(self, hidden_states):
        ...
        attention_outputs = self.apply_attention(Q, K, V)  
...
class MiniGPT1(nn.Module):
    ...
    def get_embeddings(self, inputs):
        ...
        position_indices = torch.arange(sequence_length, dtype=torch.long, device=inputs.device).unsqueeze(0).expand_as(inputs)
        embeddings = self.embedding(inputs, position_indices)
        return embeddings
    ...
    def forward(self, inputs):
        ...
        hidden_states = self.get_embeddings(inputs) 
    ...
    def loss(self, log_probas, targets, mask):
        ...
        # TODO: Write your code here
        # ==========================
        log_probas = log_probas.view(-1, log_probas.size(-1))  
        targets = targets.view(-1) 
        loss = torch.nn.functional.nll_loss(log_probas, targets, reduction='none')
        mask = mask.view(-1) 
        loss = loss * mask 
