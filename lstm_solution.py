...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        lstm_output, hidden_states = self.lstm(embeddings, hidden_states)
        log_probas = self.classifier(lstm_output)
        log_probas = F.log_softmax(log_probas, dim=-1)
    ...
    def loss(self, log_probas, targets, mask):
        ...
        log_probs_flat = torch.gather(log_probas, dim=1, index=targets.unsqueeze(1)).squeeze(1)
        mask = mask.view(-1)
        log_probs_flat = log_probs_flat[mask > 0]
        targets_flat = targets[mask > 0]
        loss = -torch.mean(log_probs_flat)
        return loss
        # pass
